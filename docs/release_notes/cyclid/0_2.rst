...
- Remove the `logout` method from the SSH transport so that we don't needlessly
  run `exit` at the end of a build.
- Always run `apt-get update` first in Ubuntu & Debian provisioners so that the
  cache is up to date before attempting to install anything.
